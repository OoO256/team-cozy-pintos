...
bool less_time_wake (const struct list_elem *lhs, const struct list_elem *rhs, void *aux);
void thread_aging(void);
bool not_greater_priority (const struct list_elem *a, const struct list_elem *b, void *aux);




bool
less_time_wake (const struct list_elem *lhs, const struct list_elem *rhs, void *aux){
  struct thread *l = list_entry (lhs, struct thread, sleep_elem);
  struct thread *r = list_entry (rhs, struct thread, sleep_elem);

  if (l->time_wake == r->time_wake)
    return l->priority > r->priority;
  else 
    return l->time_wake < r->time_wake;
}
...
thread_wake (int64_t now){
  /*
  struct list_elem *e;
  printf("[debug] sleeping list : ");
  for (e = list_begin(&sleeping_list); e != list_end(&sleeping_list); e = list_next(e))
  {
    printf("[%s] -> ", list_entry(e, struct thread, sleep_elem)->name);
  }
  printf("\n");
  */
...
thread_unblock (struct thread *t) 
{
  ...
  if (t != idle_thread)
  {
    if (thread_mlfqs)
      ...
      list_insert_ordered(&ready_list, &t->elem, not_greater_priority, NULL);

    /* printf("[debug] ready list : ");
    struct list_elem *e;
...
thread_yield (void) 
{
  ...
  if (cur != idle_thread) {
    if (thread_mlfqs)
      ...
      list_insert_ordered (&ready_list, &cur->elem, not_greater_priority, NULL);
  }
